...
**Created**: 2026-02-01
**Status**: Draft
**Input**: User description: "Wave manifest (wave.yaml) and pipeline DAG system for multi-agent orchestration wrapping Claude Code and other LLM CLIs via subprocess"

## User Scenarios & Testing _(mandatory)_

### User Story 1 - Define Project Manifest (Priority: P1)

A developer creates a `wave.yaml` file in their project root to declare the adapters (LLM CLIs), personas (agent configurations), runtime settings, and skill mounts that Wave will use. This single file is the source of truth for all orchestration behavior in the project.
...
**Independent Test**: Can be fully tested by writing a `wave.yaml`, running `wave validate`, and confirming it parses without errors and resolves all references (adapter binaries exist, persona prompt files exist, hook scripts exist).

**Acceptance Scenarios**:

1. **Given** a project directory with no Wave configuration, **When** the developer runs `wave init`, **Then** a `wave.yaml` scaffold is created with default adapter, one example persona, and runtime defaults.
2. **Given** a valid `wave.yaml` referencing a persona with a missing system prompt file, **When** the developer runs `wave validate`, **Then** the system reports the missing file path and which persona references it.
3. **Given** a `wave.yaml` with two adapters (claude, opencode), **When** the developer lists available adapters, **Then** both are shown with their binary paths, modes, and default permission sets.
...
**Why this priority**: The pipeline DAG is the core execution model. Without it, Wave is just a config file with no runtime behavior.

**Independent Test**: Can be tested by running `wave run --pipeline speckit-flow --input "add dark mode"` and observing that each step executes in order, artifacts flow between steps, and contract validation gates progression.
...
A developer runs `wave do "fix the auth bug"` for quick, single-shot tasks that don't need the full pipeline. Wave generates an in-memory two-step pipeline (navigate → execute), runs it, and discards the pipeline definition. Optionally, the generated pipeline can be saved for inspection.

**Why this priority**: Not every task justifies a multi-step pipeline. Ad-hoc mode provides the Wave safety model (personas, permissions, ephemeral workspaces) with zero configuration overhead.

**Independent Test**: Can be tested by running `wave do "fix typo in README"` and confirming a navigator runs first, then a craftsman executes the fix, all within ephemeral workspaces.

**Acceptance Scenarios**:

1. **Given** a developer runs `wave do` with a task description, **When** the command executes, **Then** a navigator analyzes the codebase first, then a craftsman implements the fix using the navigation context.
2. **Given** `wave do` with a persona override flag, **When** the command executes, **Then** the execute step uses the specified persona instead of the default craftsman.
3. **Given** `wave do` with a save flag, **When** the command completes, **Then** the generated pipeline YAML is written to the specified path for inspection or reuse.

---

### User Story 7 - Meta-Pipeline (Self-Designing) (Priority: P3)

For novel problems that don't fit existing pipeline templates, Wave's meta-pipeline has the philosopher persona design a custom pipeline at runtime. The runtime validates the generated pipeline against schema and semantic rules, then executes it. Recursion depth is capped to prevent infinite loops.
...
Wave includes a VitePress documentation site that serves as the authoritative guide for users and contributors. The documentation is comprehensive, accurate, and always in sync with the codebase. It includes getting started guides, detailed references, tutorials, examples, and architectural concepts.
...
1. **Given** a new user visits the documentation site, **When** they follow the quick-start guide, **Then** they can successfully install Wave, initialize a project, and run their first pipeline without external help.
...
3. **Given** the Wave codebase is updated, **When** CI runs, **Then** the documentation automatically updates (CLI reference from commands, schema docs from Go structs) and fails if examples are out of sync.
...
- What happens when a persona references an adapter not defined in the manifest? Validation catches this at `wave validate` time, before any pipeline runs.
...
- **FR-019**: System MUST persist ephemeral workspaces until the user explicitly runs a cleanup command (`wave clean`). Workspaces are never auto-deleted.
...
- Q: When are ephemeral workspaces cleaned up? → A: Manual only — workspaces persist until `wave clean` is run; user controls lifecycle.
...
- The host environment provides the adapter binaries on PATH; Wave does not install them.
