...
- [ ] Task 1.1: Add failure class constants to `internal/pipeline/failure.go`
...
- [ ] Task 1.2: Add retry policy presets to `internal/pipeline/types.go`
...
- [ ] Task 1.3: Add `CircuitBreaker` and `StallTimeout` config to `internal/manifest/types.go`
...
- [ ] Task 2.1: Implement failure classification logic in `internal/pipeline/failure.go` [P]
...
- [ ] Task 2.2: Implement fingerprinting and circuit breaker in `internal/pipeline/failure.go` [P]
...
- [ ] Task 2.3: Implement stall watchdog in `internal/pipeline/watchdog.go` [P]
...
- [ ] Task 3.1: Wire failure classification into executor retry loop
...
- [ ] Task 3.2: Wire circuit breaker into executor
...
- [ ] Task 3.3: Wire stall watchdog into step execution
...
- [ ] Task 3.4: Wire retry policy resolution into pipeline loading
...
- [ ] Task 4.1: Add `FailureClass` to event emission [P]
...
- [ ] Task 4.2: Bridge failure classes to recovery hints [P]
...
- [ ] Task 5.1: Unit tests for failure classification (`failure_test.go`)
...
- [ ] Task 5.2: Unit tests for fingerprinting and circuit breaker (`failure_test.go`)
...
- [ ] Task 5.3: Unit tests for stall watchdog (`watchdog_test.go`)
...
- [ ] Task 5.4: Unit tests for retry policy resolution (`types_test.go`)
...
- [ ] Task 5.5: Integration tests for executor with classification
