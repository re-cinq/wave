...
- [X] 4.4: Route on `verify.output.verdict` via the loop primitive itself rather than an explicit `branch` step: `max_iterations: 2` with **no** `loop.until` clause — the second iteration only re-runs if `verify` failed, and a single downstream `comment-back` step consumes the final verdict (pass or fail) and includes a "manual review needed" callout when fail. The inline yaml comment in `ops-pr-respond.yaml` (above the `resolve-and-verify` step) records the reason `loop.until` was not viable: it cannot reference step output values today (only pipeline context placeholders), so termination relies on `max_iterations` plus impl-finding idempotency. Collapsing to one `comment-back` leaf is simpler than branching to two near-identical leaves.
