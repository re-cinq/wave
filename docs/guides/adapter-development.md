...
type AdapterRunConfig struct {
    ...
    // Agent mode (Claude-specific; included for reference)
    UseAgentFlag bool
...
### CLAUDE.md Assembly (4 Layers)

The Claude adapter assembles `CLAUDE.md` from four layers:
...
3. **Contract compliance section** — auto-generated from step contract schema (`cfg.ContractPrompt`)
...
The Claude adapter generates `.claude/settings.json` with permissions, model, and sandbox config:
...
settings := ClaudeSettings{
    ...
    Permissions: ClaudePermissions{
        Allow: normalizeAllowedTools(cfg.AllowedTools),
        Deny:  cfg.DenyTools,
    },
}
```
