...
wave run impl-recinq

# Target a specific module
wave run impl-recinq "internal/pipeline"

# Target a specific concern
wave run impl-recinq "internal/adapter internal/manifest"
```
...
[10:19:22] + simplify completed (280s, 6.2k tokens)
  ...
  + Pipeline 'impl-recinq' completed successfully (1161s)
```
...
|----------|------|-------------|
| `findings` | `.agents/output/divergent-findings.json` | DVG-xxx findings with evidence and metrics |
| `proposals` | `.agents/output/convergent-proposals.json` | SMP-xxx prioritized proposals with 80/20 analysis |
| `result` | `.agents/output/result.md` | Summary of applied changes on worktree branch |
...
```bash
wave run impl-recinq "internal/pipeline" --to-step distill
```
...
cat .agents/workspaces/recinq/*/.agents/output/convergent-proposals.json | jq .

# If satisfied, resume from simplify
wave run impl-recinq --from-step simplify
```

## Related Use Cases

- [Refactoring](/use-cases/refactoring) - Targeted refactoring with test safety net
- [Work Supervision](/use-cases/ops-supervise) - Review quality of completed work
