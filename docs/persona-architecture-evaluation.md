...
**Rationale**: The implementer persona was created for gh-implement and plan-speckit pipelines but is functionally a subset of craftsman. Both write code, follow patterns, and produce artifacts. Maintaining two similar implementation personas creates confusion for pipeline authors.

**Changes**:
- Merge implementer's capabilities into craftsman prompt
- Add Glob and Grep to craftsman's permissions (from implementer)
- Update `plan-speckit.yaml` and `gh-implement.yaml` to use `craftsman`
