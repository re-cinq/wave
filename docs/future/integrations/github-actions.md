    ...
    steps:
      ...
      - name: Run Code Review
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
        run: |
          wave run ops-pr-review --input "Review changes in PR #${{ github.event.pull_request.number }}"
...
jobs:
  wave:
    ...
    strategy:
      matrix:
        pipeline: [ops-pr-review, lint-check, test-generation]
      fail-fast: false
    steps:
      - uses: actions/checkout@v4
...
- name: Run Review on Changed Files
  ...
  run: |
    ...
    wave run ops-pr-review --input "Review these files: $CHANGED_FILES"
```
