...
Artifacts: .agents/output/migration-report.md
```

## Complete Pipeline

Save the following YAML to `.agents/pipelines/migrate.yaml`:
...
steps:
  - id: analyze
    ...
    output_artifacts:
      - name: analysis
        path: .agents/output/migration-analysis.json
        type: json
  ...
  - id: plan
    ...
    output_artifacts:
      - name: plan
        path: .agents/output/migration-plan.md
        type: markdown
  ...
  - id: implement
    ...
    output_artifacts:
      - name: changes
        path: .agents/output/migration-changes.md
        type: markdown
  ...
  - id: verify
    ...
    output_artifacts:
      - name: verification
        path: .agents/output/migration-verification.md
        type: markdown
  ...
  - id: document
    ...
    output_artifacts:
      - name: report
        path: .agents/output/migration-report.md
        type: markdown
```
...
|----------|------|-------------|
| `analysis` | `.agents/output/migration-analysis.json` | Current state and migration scope |
| `plan` | `.agents/output/migration-plan.md` | Phased migration plan |
| `changes` | `.agents/output/migration-changes.md` | Log of implemented changes |
| `verification` | `.agents/output/migration-verification.md` | Verification results |
| `report` | `.agents/output/migration-report.md` | Complete migration documentation |

### Example Output

The pipeline produces `.agents/output/migration-report.md`:
