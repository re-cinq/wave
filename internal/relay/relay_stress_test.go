...
func TestValidateCheckpointFormat_StressTest(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:    "extremely long summary",
			content: generateLargeCheckpoint(50000, 0),
			valid:   true,
		},
		{
			name:    "many decisions",
			content: generateLargeCheckpoint(100, 1000),
			valid:   true,
		},
		{
			name:    "huge checkpoint",
			content: generateLargeCheckpoint(10000, 500),
			valid:   true,
		},
		{
			name:    "checkpoint with special characters",
			content: generateCheckpointWithSpecialChars(),
			valid:   true,
		},
		{
			name:    "checkpoint with unicode",
			content: generateCheckpointWithUnicode(),
			valid:   true,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.valid && err != nil {
				...
				t.Error("expected invalid checkpoint, got nil error")
			}

			t.Logf("Validation of %d character content took %v", len(tc.content), duration)
		})
	}
}
...
func generateLargeCheckpoint(summaryLines, decisionCount int) string {
	var summary []string
	for i := 0; i < summaryLines; i++ {
		summary = append(summary, fmt.Sprintf("Summary line %d with detailed content and context.", i+1))
	}

	var decisions []string
	for i := 0; i < decisionCount; i++ {
		decisions = append(decisions, fmt.Sprintf("Decision %d: Detailed decision with reasoning.", i+1))
	}
...
func generateCheckpointWithSpecialChars() string {
	return `# Checkpoint

## Summary
Special characters: !@#$%^&*()_+-=[]{}|;':",./<>?
Escaped chars: \"quotes\" and 'apostrophes' and \backslashes\

## Decisions
Decision with symbols: Use UTF-8 encoding & handle edge cases!

---
*Generated with special chars*
`
}

func generateCheckpointWithUnicode() string {
	return `# Checkpoint

## Summary
Unicode test: 你好 世界 🌍 🚀 ñáéíóú àèìòù äëïöü
Mathematical symbols: ∑ ∏ ∆ ∇ ∞ ≤ ≥ ≠ ± × ÷
Emoji: 🎉 🔥 ⭐ 🚀 💡 🎯 📊 🔧

## Decisions
决定: 使用Unicode字符 ✅
Décision: Utiliser des caractères accentués 🇫🇷

---
*Generated with Unicode: 生成于测试 🌟*
`
}