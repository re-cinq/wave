...
func TestWorkspaceManager_InjectArtifacts(t *testing.T) {
	...
	expectedArtifact := filepath.Join(workspacePath, ".agents", "artifacts", "step-1_output")
...
func TestWorkspaceIsolation_ArtifactInjectionIsolation(t *testing.T) {
	...
	artifactPathB := filepath.Join(wsB, ".agents", "artifacts", "step-1_secret")
	...
	artifactPathA := filepath.Join(wsA, ".agents", "artifacts", "step-1_secret")
...
// process CWD. This prevents stray .agents/workspaces/ directories from being
...
func TestNewWorkspaceManager_GitRootResolution(t *testing.T) {
	...
	wm, err := NewWorkspaceManager(".agents/workspaces")
	if err != nil {
