...
// ===========================================================================
// T030: Guided mode archive divider tests
// ===========================================================================

// newGuidedTestListModel creates a PipelineListModel in guided mode with the given data.
func newGuidedTestListModel(running []RunningPipeline, finished []FinishedPipeline) PipelineListModel {
	provider := &listTestPipelineProvider{running: running, finished: finished}
	m := NewPipelineListModel(provider)
	m.SetSize(40, 20)
	m.guided = true
	// Inject data directly
	m, _ = m.Update(PipelineDataMsg{Running: running, Finished: finished})
	return m
}

func TestPipelineListModel_Guided_RunningAndFinished_HasDivider(t *testing.T) {
	running := []RunningPipeline{
		{RunID: "r1", Name: "pipeline-a", StartedAt: time.Now()},
	}
	finished := []FinishedPipeline{
		{RunID: "f1", Name: "pipeline-b", Status: "completed", Duration: time.Minute},
	}
	m := newGuidedTestListModel(running, finished)

	hasDivider := false
	for _, item := range m.navigable {
		if item.kind == itemKindDivider {
			hasDivider = true
			break
		}
	}
	assert.True(t, hasDivider, "guided mode with running + finished should insert archive divider")
}

func TestPipelineListModel_Guided_OnlyRunning_NoDivider(t *testing.T) {
	running := []RunningPipeline{
		{RunID: "r1", Name: "pipeline-a", StartedAt: time.Now()},
		{RunID: "r2", Name: "pipeline-b", StartedAt: time.Now()},
	}
	m := newGuidedTestListModel(running, nil)

	for _, item := range m.navigable {
		assert.NotEqual(t, itemKindDivider, item.kind, "divider should not appear when there are no finished runs")
	}
}

func TestPipelineListModel_Guided_OnlyFinished_NoDivider(t *testing.T) {
	finished := []FinishedPipeline{
		{RunID: "f1", Name: "pipeline-a", Status: "completed", Duration: time.Minute},
		{RunID: "f2", Name: "pipeline-b", Status: "completed", Duration: time.Minute},
	}
	m := newGuidedTestListModel(nil, finished)

	for _, item := range m.navigable {
		assert.NotEqual(t, itemKindDivider, item.kind, "divider should not appear when there are no running pipelines")
	}
}

func TestPipelineListModel_Guided_CursorSkipsDivider_Down(t *testing.T) {
	running := []RunningPipeline{
		{RunID: "r1", Name: "pipeline-a", StartedAt: time.Now()},
	}
	finished := []FinishedPipeline{
	...
	require.GreaterOrEqual(t, dividerIdx, 0, "divider must exist to test skip behavior")

	// Navigate cursor to the item just before the divider
	for m.cursor < dividerIdx-1 {
		m, _ = sendKey(m, tea.KeyDown)
	}
	require.Equal(t, dividerIdx-1, m.cursor)

	// Press down — cursor should skip the divider
	m, _ = sendKey(m, tea.KeyDown)
	assert.NotEqual(t, itemKindDivider, m.navigable[m.cursor].kind, "cursor should skip over divider items")
}

func TestPipelineListModel_Guided_CursorSkipsDivider_Up(t *testing.T) {
	running := []RunningPipeline{
		{RunID: "r1", Name: "pipeline-a", StartedAt: time.Now()},
	}
	finished := []FinishedPipeline{
	...
	// Navigate to item after divider
	for m.cursor <= dividerIdx {
		m, _ = sendKey(m, tea.KeyDown)
	}
	require.Greater(t, m.cursor, dividerIdx)
	require.NotEqual(t, itemKindDivider, m.navigable[m.cursor].kind)

	// Press up — cursor should skip the divider
	m, _ = sendKey(m, tea.KeyUp)
	assert.NotEqual(t, itemKindDivider, m.navigable[m.cursor].kind, "cursor should skip over divider items when moving up")
}

func TestPipelineListModel_NonGuided_NoDivider(t *testing.T) {
	// Non-guided mode uses tree layout — no archive divider should appear
	running := sampleRunning(1)
	finished := sampleFinished(1)
	m := newTestListModel(running, finished, nil)
	// m.guided is false by default

	for _, item := range m.navigable {
		assert.NotEqual(t, itemKindDivider, item.kind, "non-guided mode should never insert archive divider")
	}
}

func TestPipelineListModel_Guided_RunningAppearsBeforeFinished(t *testing.T) {
	running := []RunningPipeline{
		{RunID: "r1", Name: "active-pipeline", StartedAt: time.Now()},
	}
	finished := []FinishedPipeline{
		{RunID: "f1", Name: "done-pipeline", Status: "completed", Duration: time.Minute},
	}
	m := newGuidedTestListModel(running, finished)
	...
	finishedIdx := -1
	for i, item := range m.navigable {
		if item.kind == itemKindRunning && runningIdx < 0 {
			runningIdx = i
		}
		if item.kind == itemKindFinished && finishedIdx < 0 {
			finishedIdx = i
		}
	}

	require.GreaterOrEqual(t, runningIdx, 0, "must have a running item")
	require.GreaterOrEqual(t, finishedIdx, 0, "must have a finished item")
	assert.Less(t, runningIdx, finishedIdx, "running items should appear before finished items in guided mode")
}

func TestPipelineListModel_Guided_DividerRenderedInView(t *testing.T) {
	running := []RunningPipeline{
		{RunID: "r1", Name: "active-pipeline", StartedAt: time.Now()},
	}
	finished := []FinishedPipeline{
	...
	view := listStripAnsi(m.View())
	assert.Contains(t, view, "Archive", "archive divider label should appear in guided mode view")
}
