...
2. Run `gh issue view <NUMBER> --repo <REPO> --json number,title,body,labels,url,comments` via Bash to fetch the epic
3. Run `gh issue list --repo <REPO> --json number,title,labels,url` via Bash to understand existing issues
4. Analyze the epic to identify discrete, implementable work items
5. For each sub-issue, run `gh issue create --repo <REPO> --title "<title>" --body "<body>" --label "<labels>"` via Bash
