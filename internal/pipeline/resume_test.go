...
func TestResumeManager_LoadResumeState(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fromStep: "docs",
			setupWorkspace: func(t *testing.T, tempDir string) {
				// Create completed spec workspace (hash-suffixed run dir)
				specWorkspace := filepath.Join(tempDir, ".wave/workspaces/prototype-20260312-abc123/spec")
				err := os.MkdirAll(specWorkspace, 0755)
				if err != nil {
					t.Fatal(err)
				}
		...
		},
		{
			...
			fromStep: "dummy",
			setupWorkspace: func(t *testing.T, tempDir string) {
				// Create completed spec and docs workspaces (hash-suffixed run dir)
				phases := []struct {
					phase string
					files []string
				}{
				...
				for _, phase := range phases {
					phaseWorkspace := filepath.Join(tempDir, ".wave/workspaces/prototype-20260312-abc123", phase.phase)
