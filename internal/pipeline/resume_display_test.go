...
func TestResumeFromStep_SyntheticCompletionEvents(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupWorkspace: func(t *testing.T, tempDir string) {
				for _, sid := range []string{"step-1", "step-2"} {
					dir := filepath.Join(tempDir, ".wave/workspaces/test-pipeline-20260312-abc123", sid)
		...
		},
		{
			...
			fromStep: "process",
			setupWorkspace: func(t *testing.T, tempDir string) {
				dir := filepath.Join(tempDir, ".wave/workspaces/test-pipeline-20260312-abc123/gather")
				if err := os.MkdirAll(dir, 0755); err != nil {
					t.Fatal(err)
				}
