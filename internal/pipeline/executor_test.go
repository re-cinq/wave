...
func TestAppendDefaultDeny(t *testing.T) {
	tests := []struct {
		name     string
		input    []string
		wantTodo bool
		...
		{
			name:     "empty deny list gets TodoWrite",
			input:    []string{},
			wantTodo: true,
		},
		{
			name:     "nil deny list gets TodoWrite",
			input:    nil,
			wantTodo: true,
		},
		{
			name:     "existing deny list gets TodoWrite appended",
			input:    []string{"Bash(rm -rf /*)"},
			wantTodo: true,
		},
		{
			name:     "TodoWrite already present is not duplicated",
			input:    []string{"TodoWrite", "Bash(rm -rf /*)"},
			wantTodo: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := appendDefaultDeny(tt.input)
			...
			if tt.wantTodo && !hasTodo {
				t.Errorf("expected TodoWrite in deny list, got %v", result)
			}
			if todoCount > 1 {
				t.Errorf("TodoWrite duplicated %d times in %v", todoCount, result)
			}

			// Original entries must be preserved
			for _, orig := range tt.input {
				found := false
				...
				if !found {
					t.Errorf("original deny entry %q lost in result %v", orig, result)
				}
			}
		})
	}
