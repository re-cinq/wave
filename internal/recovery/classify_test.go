...
func TestClassifyFromFailureClass(t *testing.T) {
	tests := []struct {
		name         string
		failureClass string
		expected     ErrorClass
		...
		{
			name:         "transient maps to runtime error",
			failureClass: "transient",
			expected:     ClassRuntimeError,
		},
		{
			name:         "deterministic maps to runtime error",
			failureClass: "deterministic",
			expected:     ClassRuntimeError,
		},
		{
			name:         "budget_exhausted maps to runtime error",
			failureClass: "budget_exhausted",
			expected:     ClassRuntimeError,
		},
		{
			name:         "contract_failure maps to contract validation",
			failureClass: "contract_failure",
			expected:     ClassContractValidation,
		},
		{
			name:         "test_failure maps to runtime error",
			failureClass: "test_failure",
			expected:     ClassRuntimeError,
		},
		{
			name:         "canceled maps to unknown",
		...
		{
			name:         "unrecognized class maps to unknown",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ClassifyFromFailureClass(tt.failureClass)
			if got != tt.expected {
				t.Errorf("ClassifyFromFailureClass(%q) = %q, want %q", tt.failureClass, got, tt.expected)
			}
		})
	}
}
