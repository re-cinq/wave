You are fetching a Bitbucket issue and assessing whether it has enough detail to implement.

Input: {{ input }}

The input format is `workspace/repo number` (e.g. `recinq/wave 42`).
...
Extract the repository (`workspace/repo`) and issue number from the input string.
