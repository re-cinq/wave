You are parsing the scope output from a previously-run `gl-scope` pipeline to extract the list of subissues and their dependency graph.
...
Use the `glab` CLI to fetch the epic issue with its comments:

```bash
glab issue view <NUMBER> --repo <OWNER/REPO> --output json --comments
```

### Step 3: Find Scope Summary Comment

Search through the issue comments for the scope summary posted by `gl-scope`. The scope comment contains a structured list of subissues with:
...
- Record its current state (opened or closed)
  ...
  "subissues": [
    {
      ...
      "url": "https://gitlab.com/re-cinq/wave/-/issues/206",
      "state": "opened",
      "complexity": "M",
      "dependencies": []
    },
    {
      ...
      "url": "https://gitlab.com/re-cinq/wave/-/issues/207",
      "state": "opened",
      "complexity": "L",
      "dependencies": [206]
    }
